...
const climateOn = ({ accessToken, vehicleId }) =>
    axios.post(
        `https://owner-api.teslamotors.com/api/1/vehicles/${vehicleId}/command/auto_conditioning_start`,
        {},
...
const climateOff = ({ accessToken, vehicleId }) =>
    axios.post(
        `https://owner-api.teslamotors.com/api/1/vehicles/${vehicleId}/command/auto_conditioning_stop`,
        {},
        { headers: { Authorization: `Bearer ${accessToken}` }, }
    )
...
const climate = ({ accessToken, vehicleId }) =>
    axios.get(
        `https://owner-api.teslamotors.com/api/1/vehicles/${vehicleId}/data_request/climate_state`, 
        { headers: { Authorization: `Bearer ${accessToken}` }, }
    )
